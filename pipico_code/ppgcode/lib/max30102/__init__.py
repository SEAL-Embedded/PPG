...
# Bumped from 4 → 32 so a single check() can stash a fully-drained
# sensor FIFO (max 32 entries) without overwriting its own oldest
# entries before fullpipico.py pops them. Restored to 4 if you copy
# __init__.py.bak over this file.
...
class MAX30102(object):
    ...
    def check(self):
        ...
        if read_pointer != write_pointer:
            ...
            # Drain the WHOLE FIFO before returning; the original library
            # had this return inside the for-loop so check() only ever
            # pulled one sample, which is what neutered drain-all in the
            # firmware. Restored to one-sample behavior by copying
            # __init__.py.bak over this file.
            return True
